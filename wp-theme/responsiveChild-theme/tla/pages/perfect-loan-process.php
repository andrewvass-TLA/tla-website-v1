...
?>
  <style>
    ...
    .plp-hero__copy { padding-bottom: clamp(40px, 5vw, 80px); }
    ...
    /* ── Hero composite: pages behind, Tim cropped & bleeding to bottom + off
          the right edge, floating card in front ── */
    .plp-hero__stage {
      position: relative;
      align-self: stretch;            /* fill the grid row so Tim bleeds to the bottom */
    ...
    /* plp-composite-2 — the fanned process pages, behind Tim, extending off
       the right edge of the hero */
    .plp-hero__pages {
      position: absolute;
      z-index: 1;
      right: -16%;
      ...
      width: min(118%, 760px);
      height: auto;
      filter: drop-shadow(0 24px 44px rgba(2, 28, 54, 0.32));
      pointer-events: none;
    }
    /* Tim — cropped, bleeding to the bottom of the hero, in front of the pages */
    .plp-hero__tim {
      position: absolute;
      z-index: 2;
      left: 50%;
      bottom: 0;                       /* bleeds to the hero's bottom edge */
      transform: translateX(-52%);
      width: min(94%, 460px);
      height: auto;
      filter: drop-shadow(0 18px 30px rgba(2, 28, 54, 0.28));
      pointer-events: none;
    }
    /* floating mini-card stays in front of Tim */
    .plp-minicard {
      position: absolute;
      z-index: 3;
      left: -4%;
    ...
    /* Hero composite — below the 2-col breakpoint, give the stage a fixed
       height so Tim + pages still bleed correctly under the stacked copy. */
    @media (max-width: 899px) {
      ...
      .plp-hero__pages { right: -14%; top: 6%; width: min(108%, 620px); }
      .plp-hero__tim { width: min(72%, 360px); }
    ...
    /* ═══════════════ 3-CARD ROW ═══════════════ */
    .plp-trio { margin-top: clamp(36px, 4vw, 52px); display: grid; gap: 18px; }
    @media (min-width: 760px) { .plp-trio { grid-template-columns: repeat(3, 1fr); } }
    .plp-tcard {
      border-radius: var(--plp-r-card);
    ...
    .plp-tcard__icon { width: 52px; height: 52px; border-radius: 50%; background: #fff; border: 1px solid var(--plp-line); display: flex; align-items: center; justify-content: center; margin: 0 auto 18px; }
    .plp-tcard__icon svg { width: 24px; height: 24px; stroke: var(--plp-green-deep); fill: none; stroke-width: 1.9; stroke-linecap: round; stroke-linejoin: round; }
    .plp-tcard__title { font-family: var(--font-display); font-weight: 700; font-size: 1.25rem; color: var(--plp-ink); }
    .plp-tcard__desc { font-size: 0.9rem; line-height: 1.5; color: var(--plp-muted); margin-top: 8px; }
    /* featured center card */
    .plp-tcard--feature {
      background: linear-gradient(165deg, var(--plp-mint-panel) 0%, #d8dbe0 100%);
    ...
    .plp-tcard--feature .plp-bignum { font-family: var(--font-display); font-weight: 800; font-size: clamp(2.25rem, 1.8rem + 2vw, 3rem); color: var(--plp-green-ink); line-height: 1; letter-spacing: -0.03em; font-variant-numeric: tabular-nums; }
    .plp-tcard--feature .plp-bigsub { font-size: 0.95rem; font-weight: 600; color: var(--plp-green-deep); margin-bottom: 14px; }
    .plp-tcard--feature .plp-bigsub:last-child { margin-bottom: 0; }
    ...
    /* ═══════════════ 6-CARD GRID (featured center = Tim) ═══════════════ */
    .plp-grid6 { margin-top: clamp(36px, 4vw, 52px); display: grid; gap: 18px; }
    @media (min-width: 720px) { .plp-grid6 { grid-template-columns: repeat(2, 1fr); } }
    @media (min-width: 1000px) { .plp-grid6 { grid-template-columns: repeat(3, 1fr); grid-template-rows: repeat(2, 1fr); } }
    .plp-fcard {
      background: var(--plp-card);
      border: 1px solid var(--plp-line);
      border-radius: var(--plp-r-card);
    ...
    .plp-fcard__icon { width: 64px; height: 64px; border-radius: 50%; background: #fff; border: 1px solid var(--plp-line); display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; }
    .plp-fcard__icon svg { width: 27px; height: 27px; stroke: var(--plp-green-deep); fill: none; stroke-width: 1.8; stroke-linecap: round; stroke-linejoin: round; }
    .plp-fcard__title { font-family: var(--font-display); font-weight: 700; font-size: 1.1875rem; color: var(--plp-ink); }
    .plp-fcard__desc { font-size: 0.9rem; line-height: 1.5; color: var(--plp-muted); margin-top: 8px; }
    /* featured center: spans the middle column, both rows on desktop */
    .plp-fcard--feature {
      background: linear-gradient(170deg, var(--plp-mint-panel) 0%, #d4d8dd 100%);
      border-color: transparent;
      display: flex;
      flex-direction: column;
      padding: clamp(26px, 3vw, 38px);
    }
    @media (min-width: 1000px) { .plp-fcard--feature { grid-column: 2; grid-row: 1 / span 2; } }
    .plp-fcard--feature .plp-fcard__title { color: var(--plp-green-ink); }
    .plp-fcard--feature .plp-fcard__desc { color: var(--plp-green-deep); }
    .plp-fcard__photo { margin-top: auto; position: relative; padding-top: 14px; }
    .plp-fcard__photo img { display: block; width: 100%; max-width: 230px; height: auto; margin: 0 auto; filter: drop-shadow(0 18px 26px rgba(2, 28, 54, 0.28)); }
    .plp-personcard {
      background: #fff;
      border-radius: 14px;
      box-shadow: 0 16px 34px -16px rgba(2, 28, 54, 0.4);
      padding: 12px 16px;
      margin: -16px auto 0;
      width: min(90%, 250px);
      position: relative;
    ...
    .plp-personcard__name { font-family: var(--font-display); font-weight: 700; font-size: 0.9375rem; color: var(--plp-ink); }
    .plp-personcard__role { font-size: 0.78rem; color: var(--plp-muted); }

    /* ═══════════════ PROOF (full-width testimonials + bleeding image) ═══════════════ */
    /* section crops the image at its bottom edge; no bottom padding so the
       image bleeds all the way down. */
    .plp-proof { overflow: hidden; padding-bottom: 0; }
    /* original testimonial formatting — brass left-border quotes, stacked */
    .plp-quotes { margin: clamp(28px, 3.5vw, 44px) 0 0; display: grid; gap: 18px; }
    ...
    .plp-proof__actions { margin-top: clamp(32px, 4vw, 44px); display: flex; flex-wrap: wrap; gap: 12px; justify-content: center; }
    /* large image, shown at natural proportions, full-bleed past the content.
       The section's bottom edge crops it — only its TOP portion is visible;
       the bottom is clipped where the next section begins. The negative bottom
       margin reaches past the section boundary so it's cut off, not letterboxed.
       Sized wider than the viewport so its full top half reads at a large scale. */
    .plp-proof__media {
      margin-top: clamp(-40px, -3vw, -16px);
      width: min(1600px, 130vw);
      margin-left: 50%;
      transform: translateX(-50%);
      margin-bottom: clamp(-180px, -14vw, -90px);
      line-height: 0;
    }
    .plp-proof__img {
      display: block;
...
<?php include get_stylesheet_directory() . '/tla/partials/header.php'; ?>
  ...
  <main class="site-main plp">
      ...
      <div class="container">
        <div class="plp-hero__grid">
          ...
          <div class="plp-hero__copy">
            ...
            <!-- Composite: fanned pages behind → Tim cropped, bleeds to bottom
                 → floating mini-card in front -->
            <div class="plp-hero__stage" data-hero-step aria-hidden="true">
              <span class="plp-hero__disc"></span>
              <img class="plp-hero__pages" src="<?php echo TLA_BASE; ?>/assets/plp-composite-2.png" alt="" loading="eager" />
              <img class="plp-hero__tim" src="<?php echo TLA_BASE; ?>/assets/tim-braheem-smiling.webp" alt="" loading="eager" />
    ...
    <section class="plp-section plp-section--mint" aria-labelledby="plp-installs-heading">
      <div class="container plp-shell">
        <div class="plp-prose__head" data-reveal="up">
          <span class="plp-eyebrow plp-eyebrow--center">What it is</span>
          <h2 id="plp-installs-heading" class="plp-h2 plp-h2--center" style="margin-top:14px;">72 Steps. Four Roles. One Standard for Every Loan.</h2>
        </div>
    ...
    <!-- ═══════════════ 3-CARD ROW ═══════════════ -->
    <section class="plp-section" aria-labelledby="plp-trio-heading">
      <div class="container plp-shell">
        <h2 id="plp-trio-heading" class="plp-h2 plp-h2--center" data-reveal="up">72 steps. 4 roles. One standard.</h2>
        <p class="plp-sub plp-sub--center" data-reveal="up">End-to-end ownership from first contact to post-closing — built so every client gets an experience they tell people about.</p>
        <div class="plp-trio" data-reveal="up" data-reveal-stagger="100">
          <div class="plp-tcard">
            <span class="plp-tcard__icon"><svg viewBox="0 0 24 24"><line x1="8" y1="6" x2="21" y2="6" /><line x1="8" y1="12" x2="21" y2="12" /><line x1="8" y1="18" x2="21" y2="18" /><circle cx="3.5" cy="6" r="1.5" /><circle cx="3.5" cy="12" r="1.5" /><circle cx="3.5" cy="18" r="1.5" /></svg></span>
            <div class="plp-tcard__title">Every step mapped</div>
            <p class="plp-tcard__desc">72 critical actions, in order, so nothing slips through the cracks.</p>
          </div>
          <div class="plp-tcard plp-tcard--feature">
            <div class="plp-bignum" data-countup="8.2" data-countup-prefix="$" data-countup-suffix="M">$8.2M</div>
            <div class="plp-bigsub">more closed per year vs. the industry average</div>
            <div class="plp-bignum" data-countup="72" data-countup-suffix="">72</div>
            <div class="plp-bigsub">defined steps across 4 roles</div>
          </div>
          <div class="plp-tcard">
            <span class="plp-tcard__icon"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg></span>
            <div class="plp-tcard__title">Clear role ownership</div>
            <p class="plp-tcard__desc">LO, coordinator, processor & assistant each know exactly what's theirs.</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="plp-darkband" aria-labelledby="plp-roles-heading">
      <div class="container plp-shell">
        <div class="plp-greenpanel">
          <span class="plp-eyebrow plp-eyebrow--center plp-eyebrow--on-mint" data-reveal="up">Built on the front lines</span>
          <h2 id="plp-roles-heading" class="plp-h2" style="margin-top:12px;" data-reveal="up">One playbook for your whole team</h2>
      ...
      <div class="container plp-shell">
        <div class="plp-split">
          ...
          <div class="plp-split__copy" data-reveal="up">
            <span class="plp-eyebrow">Who built it</span>
            <h2 id="plp-founder-heading" class="plp-h2" style="margin-top:14px;">Created by Tim Braheem</h2>
    ...
    <!-- ═══════════════ 6-CARD GRID (featured center = Tim) ═══════════════ -->
    <section class="plp-section" aria-labelledby="plp-grid-heading">
      <div class="container plp-shell">
        <h2 id="plp-grid-heading" class="plp-h2 plp-h2--center" data-reveal="up">Why originators swear by it</h2>
        <p class="plp-sub plp-sub--center" data-reveal="up">It wasn't built in a classroom. It was built by originating — and refined over 25+ years.</p>
        <div class="plp-grid6" data-reveal="up" data-reveal-stagger="80">
          <div class="plp-fcard">
            <span class="plp-fcard__icon"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="7" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg></span>
            <div class="plp-fcard__title">Find the right rhythm</div>
            <p class="plp-fcard__desc">A predictable cadence that works the same on file #1 and file #100.</p>
          </div>
          <!-- featured center -->
          <div class="plp-fcard plp-fcard--feature">
            <div class="plp-fcard__title">From the originator who built it</div>
            <p class="plp-fcard__desc">$1.4B in personal volume. The Perfect Loan Process is the system behind it.</p>
            <div class="plp-fcard__photo">
              <img src="<?php echo TLA_BASE; ?>/assets/tim-braheem-smiling.webp" alt="Tim Braheem" loading="lazy" />
            </div>
            <div class="plp-personcard">
              <div class="plp-personcard__name">Tim Braheem</div>
              <div class="plp-personcard__role">Co-founder, The Loan Atlas</div>
            </div>
          </div>
          <div class="plp-fcard">
            <span class="plp-fcard__icon"><svg viewBox="0 0 24 24"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg></span>
            <div class="plp-fcard__title">Stop the scramble</div>
            <p class="plp-fcard__desc">No more files falling apart mid-process and last-minute heroics.</p>
          </div>
          <div class="plp-fcard">
            <span class="plp-fcard__icon"><svg viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg></span>
            <div class="plp-fcard__title">Communicate proactively</div>
            <p class="plp-fcard__desc">End the anxious "what's happening?" calls before they start.</p>
          </div>
          <div class="plp-fcard">
            <span class="plp-fcard__icon"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12" /></svg></span>
            <div class="plp-fcard__title">Earn the referral</div>
            <p class="plp-fcard__desc">A post-closing sequence designed to turn clients into advocates.</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- ═══════════════ PROOF (full-width) ═══════════════ -->
    <section class="plp-section plp-section--mint plp-proof" aria-labelledby="plp-proof-heading">
      <div class="container plp-shell">
        <h2 id="plp-proof-heading" class="plp-h2 plp-h2--center" data-reveal="up">What Happens When You Have a Perfect Loan Process</h2>
        ...
        <div class="plp-proof__actions" data-reveal="up">
          <a class="btn btn--gold btn--lg" href="#get-the-plp" data-scroll-to-form>Start Building Your Perfect Loan Process →</a>
        </div>
      </div>
      <div class="plp-proof__media" data-reveal="up" aria-hidden="true">
        <img class="plp-proof__img" src="<?php echo TLA_BASE; ?>/assets/plp-composite-2.png" alt="" loading="lazy" />
      </div>
    </section>

    <!-- ═══════════════ THE FOUNDATION ═══════════════ -->
    <section class="plp-section" aria-labelledby="plp-foundation-heading">
      <div class="container plp-shell">
        <div class="plp-prose">
          <div class="plp-prose__head">
            <span class="plp-eyebrow plp-eyebrow--center" data-reveal="up">The foundation</span>
            <h2 id="plp-foundation-heading" class="plp-h2 plp-h2--center" style="margin-top:14px;" data-reveal="up">The Foundation for Every Top-Producing Originator</h2>
          </div>
          <div class="plp-prose__body" data-reveal="up">
            <p>The Perfect Loan Process isn't one lesson inside The Loan Atlas. It's the foundation everything else is built on.</p>
            <p>Every system, every script, every coaching session inside the platform traces back to the same idea: that a great mortgage business is run on systems. The scripts that make the baton passes clean. The communication templates that make clients feel taken care of before they think to call. The AI tools that help you execute it consistently. They all sit on top of this framework.</p>
            <p class="plp-prose__payoff">That's why the originators who build their business around it don't just close more loans — they close more <em>predictably</em>. <strong>Loan Atlas members close an average of $8.2M more per year</strong> than the industry average.</p>
            <p>Once the process is in place, your mortgage business will run itself.</p>
          </div>
        </div>
      </div>
    </section>
