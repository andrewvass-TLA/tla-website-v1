<?php
/**
 * Body partial for /certainty-home-lending/ (TLA Full HTML template).
 * Generated from public/custom-sales-pages/certainty-home-lending.html by scripts/convert-pages.sh — do not hand-edit;
 * edit the source HTML (or the shared header/footer partials) and re-run.
 */
if ( ! defined( 'ABSPATH' ) ) { exit; }

$tla_title       = 'CHL Preferred Account — Exclusive Loan Atlas Pricing';
$tla_description = 'CHL Preferred Account pricing: full access to The Loan Atlas at $99/mo — down from $349/mo — a $3,000 annual savings, exclusively for CHL originators.';
$tla_active      = '';
?>
  <style>
    ...
    /* ── Sticky offer bar — pinned under the header ─────────────────────────────
       Two tiers: a thin brass strip naming the audience, and a dark row carrying
       the numbers. The dark row continues the header's gradient so the bar reads
       as part of the chrome rather than a banner stapled below it. ──────────── */
    .apm-bar {
      position: sticky;
      top: var(--header-h);
      z-index: 40;
      border-bottom: 2px solid transparent;
      border-image: linear-gradient(90deg, #c9961c, #ffd56c, #c9961c) 1;
      box-shadow: 0 8px 26px rgba(2, 28, 54, 0.28);
    }

    /* The whole bar is one link to checkout. It stays a plain block that
       inherits its children's colors, so the two tiers look identical to the
       pre-link version — only the cursor, a subtle brightening on hover and the
       focus ring signal that it's clickable. No hover lift on purpose: the bar
       is sticky, so shifting it would slide it against the scrolling page. */
    .apm-bar__link {
      display: block;
      color: inherit;
    ...
    .apm-bar__link:focus-visible {
      outline: 3px solid #ffd56c;
      outline-offset: -3px;
    }
    @media (prefers-reduced-motion: reduce) {
      .apm-bar__link { transition: none; }
    }

    /* Tier 1 — brass audience strip. The rules either side of the label are
       pseudo-elements so the markup stays a single span. */
    .apm-bar__strip {
    ...
    .apm-bar__strip-in::before,
    .apm-bar__strip-in::after {
      content: "";
      height: 1px;
      width: clamp(20px, 6vw, 70px);
      background: rgba(2, 28, 54, 0.32);
    }
    .apm-bar__label {
    ...
    /* Tier 2 — dark detail row.
       Two nested axes so the copy and the pill are both honest:
       - .apm-bar__row-in centres its children, so the text group and the CTA
         share one vertical centre line.
       - .apm-bar__copy inside it is baseline-aligned, so the price, offer and
         savings sit on one optical line. */
    .apm-bar__row {
    ...
    /* The + is its own element, centred between the two facts. It is
       align-self: center rather than on the shared baseline because the glyph's
       ink sits high in its em box — baselining it floats it above centre. */
    .apm-bar__plus {
      align-self: center;
      font-family: var(--font-display);
      font-weight: 700;
    ...
    .apm-bar__free b { color: #eac25a; font-weight: 800; }
    .apm-bar__save {
      font-family: var(--font-display);
      font-weight: 800;
      font-size: 1.3125rem;
      ...
      white-space: nowrap;
      line-height: 1.1;
      margin-left: clamp(7px, 1.3vw, 16px);
    }
    /* Step the type down so the row holds one line as long as possible. */
    @media (max-width: 1279px) {
      .apm-bar__label { font-size: 0.875rem; letter-spacing: 0.15em; }
      .apm-bar__row-in { padding: 15px var(--gutter); }
      .apm-bar__price { font-size: 1.625rem; }
      .apm-bar__free, .apm-bar__save { font-size: 1.125rem; }
      .apm-bar__plus { font-size: 1.3125rem; }
    }
    @media (max-width: 1099px) {
      .apm-bar__label { font-size: 0.8125rem; letter-spacing: 0.14em; }
      .apm-bar__price { font-size: 1.375rem; }
      .apm-bar__free, .apm-bar__save { font-size: 1rem; }
      .apm-bar__plus { font-size: 1.1875rem; }
    }
    @media (max-width: 899px) {
      .apm-bar__strip-in { padding: 6px var(--gutter); }
      .apm-bar__label { font-size: 0.75rem; letter-spacing: 0.12em; }
      .apm-bar__row-in { padding: 13px var(--gutter); }
      .apm-bar__price { font-size: 1.1875rem; }
      .apm-bar__free, .apm-bar__save { font-size: 0.875rem; }
      .apm-bar__plus { font-size: 1rem; }
    }
    /* Below 640px the offer copy takes its own centred line, so the standalone
       + would be orphaned beside the price — hide it and inline it instead. */
    @media (max-width: 639px) {
      .apm-bar__label { font-size: 0.625rem; letter-spacing: 0.06em; }
      .apm-bar__strip-in::before,
      .apm-bar__strip-in::after { width: clamp(12px, 4vw, 40px); }
      .apm-bar__row-in { padding: 9px 14px; gap: 8px 12px; }
      .apm-bar__copy { gap: 4px 12px; }
      .apm-bar__price { font-size: 1.25rem; }
      .apm-bar__price s { display: none; }
      .apm-bar__plus { display: none; }
      .apm-bar__free {
        flex-basis: 100%;
        text-align: center;
        font-size: 0.8125rem;
      ...
      .apm-bar__free::before {
        content: "+ ";
        color: rgba(234, 194, 90, 0.85);
        font-weight: 700;
      }
      .apm-bar__save { font-size: 0.8125rem; margin-left: 0; }
    }
    /* Narrowest phones: the label is wider than the strip at full tracking. */
    @media (max-width: 380px) {
      .apm-bar__label { font-size: 0.625rem; letter-spacing: 0.06em; }
      .apm-bar__strip-in { gap: 8px; }
      .apm-bar__strip-in::before,
      .apm-bar__strip-in::after { display: none; }
    }
    ...
    /* Larger hero body copy (matches the homepage hero). The shared rule caps
       this at 40rem, which pinches this page's longer subtitle well inside the
       headline above it — widen it to sit with the 56rem content column. */
    .lp-intro__subtitle {
      font-size: clamp(1.125rem, 0.95rem + 0.6vw, 1.375rem);
      max-width: 52rem;
    }
    /* "With Less Chaos" — brass gold gradient, always on its own line. */
    .mm-hero-gold {
      display: block;
      background: linear-gradient(135deg, #c9961c 0%, #eac25a 50%, #ffd56c 100%);
      -webkit-background-clip: text;
    ...
    /* Lockup + offer-name title inside the offer card */
    ...
    .mm-step--first .mm-step__price {
      font-size: clamp(3.75rem, 2.3rem + 7vw, 6.25rem);
      background: linear-gradient(135deg, #c9961c 0%, #eac25a 50%, #ffd56c 100%);
      -webkit-background-clip: text;
      background-clip: text;
      color: transparent;
    ...
    .system-card p {
      font-size: clamp(1.03125rem, 0.98rem + 0.22vw, 1.125rem);
      line-height: 1.62;
    ...
    @media (min-width: 760px) {
      /* The price column needs more than half the box: "3 Months FREE" is
         nowrap at up to 68px. Give it the larger share and put the divider on
         the real column boundary rather than at 50%, so the price can never
         cross it. --plat-split is that boundary, shared by both. */
      .mm-plat__offer {
        --plat-split: 44%;
      ...
      .mm-plat__price-note { text-align: left; }
    ...
    .mm-plat__price-was {
      ...
      font-family: var(--font-display);
      font-weight: 700;
      font-size: clamp(2.25rem, 1.7rem + 1.7vw, 3.25rem);
      color: rgba(255, 255, 255, 0.5);
    }
    ...
    /* Strike only the figure, not the trailing "/mo" — and keep it level.
       The rule used to span the whole element at -8deg, which overshot the
       digits once the price was enlarged. */
    ...
    .mm-plat__price-now {
      font-family: var(--font-display);
      font-weight: 800;
      font-size: clamp(2.25rem, 1.1rem + 2.6vw, 3.375rem);
      line-height: 0.95;
      letter-spacing: -0.03em;
      white-space: nowrap;
      background: linear-gradient(135deg, #c9961c 0%, #eac25a 50%, #ffd56c 100%);
    ...
    /* Footnote for the caption's asterisk. Its own full-width row at the base
       of the card, pulled toward the bottom padding edge, and kept to a single
       line — no max-width, and it shrinks rather than wrapping on narrow
       screens (see the nowrap + clamp below). */
    .lp-proof-card__footnote {
      /* Positioned rather than a grid row: as a row it inherited the card's
         56px row-gap above it, which no negative margin could collapse. */
      position: absolute;
      left: clamp(24px, 4vw, 56px);
      right: clamp(24px, 4vw, 56px);
      bottom: clamp(12px, 1.4vw, 18px);
      font-family: var(--font-body);
      font-size: clamp(0.625rem, 0.36rem + 0.7vw, 0.75rem);
      font-weight: 400;
      line-height: 1.3;
    ...
    @media (min-width: 760px) {
      .lp-proof-card__footnote { text-align: left; }
    }
    /* Phones: one line is impossible at a readable size — wrap instead. */
    ...
    /* ── Embedded CHL enrollment form (nested inside the pricing card) ──────── */
    .apm-form {
      position: relative;
      z-index: 1;
      margin-top: var(--space-lg);
      /* Fill (nearly) the whole card. The embedded form has no internal width cap,
         so it grows with the iframe as the box widens. */
      max-width: 48rem;
      margin-inline: auto;
      background: #ffffff;
      border: 1px solid rgba(234, 194, 90, 0.4);
      border-radius: var(--radius-2xl);
      box-shadow: inset 0 0 0 1px rgba(234, 194, 90, 0.12);
    ...
    /* The LeadConnector embed script sizes the iframe to the form's own content
       height (~532px per the embed); keep a floor so there's no big empty gap
       below the form before the script runs. */
    .apm-form__embed iframe {
      display: block;
      width: 100%;
      min-height: 540px;
    }
    /* Savings pill (ACM-style) — replaces the plain italic savings line. */
    .mm-plan__save-pill {
      display: inline-block;
      font-family: var(--font-body);
    ...
    /* With the pill, the savings line is upright and needs normal flow spacing
       (CHL's base .mm-plan__save pulls itself up under a big hero title).
       The gap above/below must read as equal, so this rule owns both sides:
       the headline's margin-bottom and the form's margin-top are both zeroed
       out below, leaving only these two values in play. */
    .mm-plan__save.mm-plan__save--pill {
      font-style: normal;
      color: inherit;
      /* Two classes: the base .mm-plan__save is declared later in this sheet
         and its negative margin-top would otherwise win. */
      margin-top: var(--space-lg);
      margin-bottom: var(--space-lg);
      margin-inline: auto;
    }
    /* Collapse the neighbours' own spacing into the pill's, so the visible gap
       is symmetric. Without this the headline contributed ~12px above while
       --space-xl + .apm-form's margin-top stacked to ~42px below.
       Both cards order these three elements identically, so plain sibling
       selectors are enough — no :has() needed. */
    .mm-offer__card .mm-plan__hero-title,
    .mm-plan .mm-plan__hero-title {
      margin-bottom: 0;
    }
    ...
    /* ── Price build — how the $99 is made up ────────────────────────────────
       A parenthetical under the headline: the discounted rate plus CHL's
    ...
    .lp-proof-card {
      ...
      /* Extra bottom room for the absolutely-positioned footnote. */
    ...
    /* Narrow phones: the footnote wraps to 2–3 lines, which needs more reserved
       space at the card's base than the desktop single line. */
    @media (max-width: 430px) {
      .lp-proof-card { padding-bottom: 62px; }
    }
    @media (max-width: 380px) {
      .lp-proof-card { padding-bottom: 76px; }
    }
    ...
    /* Logo over a single-line account heading. No divider rule between them —
       the gap alone separates the mark from the title. */
    .mm-plan__lockup {
      display: flex;
      flex-direction: column;
      align-items: center;
    ...
    /* Account-tier heading — unused: the "CHL Preferred Account" heading was
       removed from both pricing cards (the headline below states the offer).
       Rule kept in case the lockup heading returns. */
    .mm-plan__lockup-label {
      display: block;
      font-family: var(--font-display);
      font-size: clamp(0.9375rem, 0.5rem + 2.4vw, 2.875rem);
      font-weight: 800;
      letter-spacing: -0.015em;
      line-height: 1.15;
      color: #ffffff;
    ...
    /* Sub-note under the plus line. Sits close beneath it (the plus line owns a
       large auto margin, so cancel most of the bottom half) and drops to body
       face at a smaller size so it reads as a footnote to that line. */
    .mm-plan__plus-note {
      font-family: var(--font-body);
      font-weight: 400;
      font-size: clamp(1rem, 0.93rem + 0.25vw, 1.0625rem);
      line-height: 1.45;
      color: rgba(255, 255, 255, 0.62);
      text-align: center;
      max-width: 34rem;
      margin: calc(var(--space-lg) * -0.6) auto var(--space-lg);
    }
    .mm-plan__plus-note strong {
      font-weight: 800;
      color: rgba(255, 255, 255, 0.92);
    }
    ...
    /* Savings line — sits under the card heading. No pill chrome; italic so it
       reads as an aside to the heading rather than a second label. The lockup
       above owns a large bottom margin, so pull up into it. */
    .mm-plan__save {
      text-align: center;
      font-family: var(--font-body);
      font-style: italic;
      ...
      color: #6ee7a8;
      max-width: 46rem;
      margin: calc(var(--space-lg) * -0.7) auto clamp(28px, 3.4vw, 48px);
    }
    .mm-plan__save em {
      font-style: italic;
      font-weight: 800;
    ...
    /* ══════════════════════════════════════════════════════════════════════════
       CHL EXCLUSIVE PRICING — two-tier monthly/annual card.
       Ported from public/custom-sales-pages/namb-conference-2026.html so both pricing
       sections on this page present the identical CHL member offer.
...
<?php include get_stylesheet_directory() . '/tla/partials/header-minimal.php'; ?>
  ...
  <!-- ── Sticky offer bar ───────────────────────────────────────────────────── -->
  <div class="apm-bar">
    <a class="apm-bar__link" href="#apm-form" aria-label="Activate Your CHL Preferred Account — $99/mo, save $3,000 a year">
      <div class="apm-bar__strip">
        <div class="apm-bar__strip-in">
          <span class="apm-bar__label">Activate Your CHL Preferred Account</span>
        </div>
      </div>
      <div class="apm-bar__row">
        <div class="apm-bar__row-in">
          <div class="apm-bar__copy">
            <span class="apm-bar__price"><s>$349/mo</s>$99<i>/mo</i></span>
            <span class="apm-bar__save">Save $3,000/yr</span>
          </div>
        </div>
      </div>
    </a>
  ...
  <main>
    ...
    <section class="lp-intro" aria-labelledby="mm-hero-heading">
      <div class="container">
        <div class="lp-intro__content">
          ...
          <p class="lp-intro__subtitle" data-hero-step="3">The Loan Atlas installs 5 AI-powered systems into your mortgage business to help you make better decisions, execute faster, and stay focused on what actually drives production.</p>
          <div class="lp-intro__actions" data-hero-step="4">
            <a class="btn btn--gold btn--lg" href="#apm-form">Activate Your CHL Preferred Account</a>
          </div>
        </div>
      </div>
    ...
    <section class="mm-offer" aria-labelledby="mm-offer-heading">
      <div class="container">
        <div id="apm-form" class="mm-offer__card" data-reveal="scale">
          <div class="mm-plan__lockup" aria-label="The Loan Atlas — CHL Preferred Account pricing">
            <img class="mm-plan__lockup-atlas" src="<?php echo TLA_BASE; ?>/assets/Loan Atlas logo-gold.png" alt="The Loan Atlas" />
          </div>
          <h3 id="mm-offer-heading" class="mm-plan__hero-title mm-offer__hero-title" style="color: #ffffff;">Complete the Form to Activate Your CHL Preferred Account for just <span style="background: linear-gradient(135deg, #c9961c 0%, #eac25a 50%, #ffd56c 100%); -webkit-background-clip: text; background-clip: text; color: transparent;">$99/mo</span></h3>

          <!-- How the $99 is made up: the discounted rate, plus CHL's subsidy. -->
          <p class="apm-build">(<span class="apm-build__amount">$199<i>/mo</i></span> discounted rate <span class="apm-build__op">+</span>&nbsp;<span class="apm-build__amount">$100<i>/mo</i></span> subsidy from CHL)</p>

          <!-- Savings pill — hidden for review. Uncomment to bring it back.
          <p class="mm-plan__save mm-plan__save--pill"><span class="mm-plan__save-pill"><em>$3,000</em> in Annual Savings — Exclusively for CHL Originators</span></p>
          -->

          <!-- ── Embedded CHL enrollment form (inside the top offer card) ─────── -->
          <div class="apm-form">
            <div class="apm-form__head">
              <h4 class="apm-form__title">Activate Your CHL Preferred Account</h4>
              <p class="apm-form__sub">Complete the form below and our team will get you set up at $99/mo.</p>
            </div>
            <div class="apm-form__embed">
              <iframe
                src="https://api.leadconnectorhq.com/widget/form/3ZQFIuaR5CEVCPxazkCr"
                style="width:100%;height:100%;border:none;border-radius:3px"
                id="inline-3ZQFIuaR5CEVCPxazkCr-top"
                ...
                data-form-name="CA: CHL"
                data-height="532"
                data-layout-iframe-id="inline-3ZQFIuaR5CEVCPxazkCr-top"
                data-form-id="3ZQFIuaR5CEVCPxazkCr"
      ...
      <div class="container">
        <div class="lp-bridge__stack">
          ...
          <div class="lp-bridge__inner" data-reveal="up">
            ...
            <p class="text-brass" style="font-family: var(--font-display); font-size: clamp(1.5rem, 1.1rem + 1.4vw, 2.125rem); font-weight: 800; line-height: 1.35; letter-spacing: -0.015em; margin: 0;">
              With your CHL Preferred Account, you can install them all for less than the commission of one extra loan.
            </p>
          </div>
    ...
    <!-- NOTE: the Platinum Marketing section (was 7b) is intentionally removed —
         the CHL Preferred Account is $99/mo flat with no free-months bonus, so
         advertising it here would promise something the offer doesn't include.
         Its .mm-plat__* CSS above is left in place (same as acm.html). -->
    ...
    <section class="lp-testimonials" id="testimonials" aria-labelledby="testimonials-heading">
      <div class="container">
        ...
        <div class="lp-proof-card" data-reveal="up" style="margin-top: clamp(32px, 4vw, 56px);">
          ...
          <p class="lp-proof-card__footnote">* Past 12 months, ending 2/15/2026. Must have closed one loan in last 90 days (154,000 LO population).</p>
        </div>
      </div>
    </section>

    <!-- ── 12. CHL PRICING ──────────────────────────────────────────────────── -->
    <section class="mm-pricing" id="pricing" aria-label="CHL Preferred Account pricing">
      <div class="container">
        <article id="apm-pricing-card" class="mm-plan" data-reveal="scale">
          <div class="mm-plan__lockup" aria-label="The Loan Atlas — CHL Preferred Account pricing">
            <img class="mm-plan__lockup-atlas" src="<?php echo TLA_BASE; ?>/assets/Loan Atlas logo-gold.png" alt="The Loan Atlas" />
          </div>
          <h3 class="mm-plan__hero-title mm-offer__hero-title" style="color: #ffffff;">Complete the Form to Activate Your CHL Preferred Account for just <span style="background: linear-gradient(135deg, #c9961c 0%, #eac25a 50%, #ffd56c 100%); -webkit-background-clip: text; background-clip: text; color: transparent;">$99/mo</span></h3>
          ...
          <!-- ── Embedded CHL enrollment form (inside the pricing card) ──────── -->
          <div class="apm-form">
            <div class="apm-form__head">
              <h4 class="apm-form__title">Activate Your CHL Preferred Account</h4>
              <p class="apm-form__sub">Complete the form below and our team will get you set up at $99/mo.</p>
            ...
            <div class="apm-form__embed">
              <iframe
                src="https://api.leadconnectorhq.com/widget/form/3ZQFIuaR5CEVCPxazkCr"
                style="width:100%;height:100%;border:none;border-radius:3px"
                id="inline-3ZQFIuaR5CEVCPxazkCr"
                data-layout="{'id':'INLINE'}"
                data-trigger-type="alwaysShow"
                data-trigger-value=""
                data-activation-type="alwaysActivated"
                ...
                data-form-name="CA: CHL"
                data-height="532"
                data-layout-iframe-id="inline-3ZQFIuaR5CEVCPxazkCr"
                data-form-id="3ZQFIuaR5CEVCPxazkCr"
                data-cookie-consent="true"
                data-cookie-consent-provider="auto"
                title="CA: CHL">
...
<?php include get_stylesheet_directory() . '/tla/partials/footer.php'; ?>
  ...
  <!-- LeadConnector form embed (CHL enrollment form) -->
