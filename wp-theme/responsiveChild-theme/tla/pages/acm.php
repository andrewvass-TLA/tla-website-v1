<?php
/**
 * Body partial for /acm/ (TLA Full HTML template).
 * Generated from public/custom-sales-pages/acm.html by scripts/convert-pages.sh — do not hand-edit;
 * edit the source HTML (or the shared header/footer partials) and re-run.
 */
if ( ! defined( 'ABSPATH' ) ) { exit; }

$tla_title       = 'ACM Preferred Account — Exclusive Loan Atlas Pricing';
$tla_description = 'ACM Preferred Account pricing: full access to The Loan Atlas at $199/mo locked in — down from $349/mo.';
$tla_active      = '';
?>
  <style>
    ...
    .site-header--acm .site-header__inner {
    ...
    @media (min-width: 540px) { .acm-nav-lockup__acm { height: 30px; } }
    @media (min-width: 880px) { .acm-nav-lockup__acm { height: 34px; } }
    /* On smaller screens, use the short button label so it fits beside the logo. */
    @media (max-width: 767.98px) {
      .site-header--acm .btn--header .btn__short { display: inline; }
      .site-header--acm .btn--header .btn__full { display: none; }
    }
    /* On very small screens, drop the ACM wordmark so the logo + button still fit. */
    @media (max-width: 559.98px) {
      .site-header--acm .acm-nav-lockup__divider,
      .site-header--acm .acm-nav-lockup__acm { display: none; }
    }
...
<?php include get_stylesheet_directory() . '/tla/partials/header-minimal.php'; ?>
  ...
  <main>
    ...
    <section class="lp-intro" aria-labelledby="mm-hero-heading">
      <div class="container">
        <div class="lp-intro__content">
          <p class="acm-hero-eyebrow" data-hero-step="1">ACM Preferred Account</p>
          ...
          <div class="lp-intro__actions" data-hero-step="4">
            <a class="btn btn--gold btn--lg" href="#acm-form">Claim Your ACM Preferred Membership</a>
          </div>
        </div>
      </div>
    ...
    <section class="mm-offer" aria-labelledby="mm-offer-heading">
      <div class="container">
        <div id="acm-form" class="mm-offer__card" data-reveal="scale">
          ...
          <div class="acm-form">
            ...
            <div class="acm-form__embed">
              <iframe
                ...
                id="inline-iZbKtYJN0Ia5ayRrxOwU-top"
                ...
                data-height="754"
                data-layout-iframe-id="inline-iZbKtYJN0Ia5ayRrxOwU-top"
        ...
        <div class="lp-bridge__stack">
          ...
          <div class="lp-bridge__inner" data-reveal="up">
            ...
            <p class="text-brass" style="font-family: var(--font-display); font-size: clamp(1.5rem, 1.1rem + 1.4vw, 2.125rem); font-weight: 800; line-height: 1.25; letter-spacing: -0.015em; margin: 0;">
              With your ACM Preferred Pricing, you can install them all for less than the commission of one extra loan.
            </p>
          </div>
    ...
    <section class="mm-pricing" id="pricing" aria-labelledby="pricing-heading">
      <div class="container">
        <div class="center" data-reveal="up" style="max-width: 46rem; margin-inline: auto;">
          <h2 id="pricing-heading" class="t-display" style="color: #ffffff; font-weight: 800; margin-bottom: var(--space-sm);">Everything You Need to Scale. <span style="background: linear-gradient(135deg, #c9961c 0%, #eac25a 50%, #ffd56c 100%); -webkit-background-clip: text; background-clip: text; color: transparent;">Exclusive ACM Pricing.</span></h2>
          <p class="t-body-lg" style="color: rgba(255, 255, 255, 0.68); font-size: clamp(1.125rem, 0.95rem + 0.6vw, 1.375rem);">Full access to everything above — the AI systems, live coaching, the curriculum, the community — at exclusive ACM Preferred Account pricing.</p>
        </div>

        <article id="acm-pricing-card" class="mm-plan" data-reveal="scale">
          <div class="mm-plan__lockup" aria-label="The Loan Atlas — ACM Preferred Account pricing">
            <img class="mm-plan__lockup-atlas" src="<?php echo TLA_BASE; ?>/assets/Loan Atlas logo-gold.png" alt="The Loan Atlas" />
            <span class="mm-plan__lockup-x" aria-hidden="true"></span>
            <img class="mm-plan__lockup-mm" src="<?php echo TLA_BASE; ?>/assets/acm-logo-primary-blue.webp" alt="Atlantic Coast Mortgage" />
...
<?php include get_stylesheet_directory() . '/tla/partials/footer.php'; ?>
  ...
  <!-- Add the ACM logo beside the Loan Atlas logo, and a "Claim" CTA button, into
  ...
  <script>
    (function () {
      ...
      if (!header || !brand || document.querySelector('.acm-nav-lockup__acm')) return;
      var logo = brand.querySelector('.brand__logo');
      if (!logo) return;
      // Flag the header so its layout switches from centered to space-between.
      header.classList.add('site-header--acm');
      // Derive the ACM asset URL from the (already correctly-resolved) Loan Atlas
      // logo src, so it works whether the base is /assets/ (built) or ./assets/ (raw).
      var acmSrc = logo.getAttribute('src').replace(/[^/]+$/, 'acm-logo-primary-blue.webp');
      var divider = document.createElement('span');
      divider.className = 'acm-nav-lockup__divider';
      divider.setAttribute('aria-hidden', 'true');
      var acm = document.createElement('img');
      acm.className = 'acm-nav-lockup__acm';
      acm.src = acmSrc;
      acm.alt = 'Atlantic Coast Mortgage';
      // Group brand + divider + ACM logo into one lockup so space-between keeps
      // them together on the left (button goes on the right).
      var lockup = document.createElement('div');
      lockup.className = 'acm-nav-lockup';
      brand.parentNode.insertBefore(lockup, brand);
      lockup.appendChild(brand);
      lockup.appendChild(divider);
      lockup.appendChild(acm);
      ...
      cta.href = '#acm-form';
      cta.innerHTML = '<span class="btn__short">Claim Membership</span><span class="btn__full">Claim Your ACM Membership</span>';
